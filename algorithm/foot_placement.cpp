...
void FootPlacement::getSwingPos()
{
    ...
    // for linear velocity - improved for better terrain adaptation
    ...
    // Improved terrain height estimation - use stance foot as reference
    double terrainHeightEstimate = STPos_W(2);

    // Calculate desired landing height with better terrain adaptation
    // Use stance foot height as baseline, allowing for terrain changes
    double heightDifference = base_pos(2) - legLength + zOff_W - terrainHeightEstimate;

    // Limit height difference to prevent excessive drops
    heightDifference = std::max(heightDifference, -0.25); // Max 25cm drop
    heightDifference = std::min(heightDifference, 0.15);  // Max 15cm rise

    posDes_W(2) = terrainHeightEstimate + heightDifference;
    ...
    // cycloid trajectories for x-y plane
    if (phi < 1.0)
    {
    ...
    // Improved foot stretch for better ground contact
    if (phi >= 0.98)
    {
        zStretch += -0.003; // Slightly more aggressive stretch
    }
    else
        zStretch = 0;
    if (zStretch < -0.06) // Allow more stretch for steep terrain
    {
        zStretch = -0.06;
    }

    // Improved vertical trajectory with better clearance for stairs
    pDesCur[2] = posStart_W(2) + Trajectory(0.25, stepHeight, posDes_W(2) - posStart_W(2)) + zStretch;
}
